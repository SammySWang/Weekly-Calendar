...
function writeSearch(req, res) {
    ...
    <html lang = "en">
    <head>
        <title>Spring 2021 CSE Course Search</title>
    </head>
    
    <body>
    <h1>Spring 2021 CSE Course Search</h1><br>
    ...
    if (filter == "allFields")
        ...
        sql = `SELECT * FROM courses
            ...
            ORDER BY STR_TO_DATE(StartTime,'%h:%i %p');`;
    
    //ORDER BY STR_TO_DATE(StartTime,'%h.%i%p');`;
    //ORDER BY StartTimeInternal;`;
    else if (filter == "time")
        sql = `SELECT * FROM courses
            WHERE StartTime     LIKE '%` + search + `%' OR EndTime     LIKE '%` + search + `%';`;
    ...
    con.query(sql, function(err, result){
       ...
       for (let item of result){
           html += `
           <button type = "button" class = "toggle">CSE `+item.Course + ` - ` + item.CourseName + ` - `+ item.Component + ` - Section ` + item.Section + `</button>
           <pre>
           Days: ` + item.Days + `
        ...
        con.query(constructSQLDayCommand("TU"), function(err, result){
            ...
            con.query(constructSQLDayCommand("W"), function(err, result){
                ...
                con.query(constructSQLDayCommand("TH"), function(err, result){
                    if(err) throw err;
                    html =  html.replace("<td> Thu </td>", getDay(result, "TH"));
...
function getDay(SQLResult, tableHeader){
    ...
    for(let item of SQLResult){
        retStr += "\n      <b>" + item.StartTime + " - " + item.EndTime + "<br><br>" + item.Subject + " - " +item.Course + " - " + item.Section + "<b><p> " + item.CourseName + "<br><br>" + item.Instructor + "<br><br>" + "<br/><br/>";
    }
